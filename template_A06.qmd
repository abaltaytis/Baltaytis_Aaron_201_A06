...
[https://github.com/abaltaytis/Baltaytis_Aaron_201_A06](https://github.com/abaltaytis/Baltaytis_Aaron_201_A06)
...
```{r}
# Create a new list for the players' respective heights
heights <- list(
  "MJ" = 78,
  "LJ" = 81,
  "KB" = 78,
...
# Iterate through to add their heights
for(ballers in 1:length(player_info)){
  player_info[[ballers]]$height <- heights[[ballers]]
}
player_info
```
...
for(ballers in names(player_info)){
  points_per_game <- player_info[[ballers]]$points/player_info[[ballers]]$games_played
ppg[[ballers]] <- points_per_game
}
print(ppg)
```
...
round_mat <- apply(mat_rand, MARGIN = 1:2, FUN = round_fun)
print(round_mat)
```
...
state_OH <- max_temps[max_temps$state == "OH",]
state_VA <- max_temps[max_temps$state == "VA",]
```
...
```{r}
med_temp_CO <- apply(state_CO[, 3:14], 1, median)

# Assign vars so paste0 is cleaner
max_yr <- state_CO[which.max(med_temp_CO), 2]
min_yr <- state_CO[which.min(med_temp_CO), 2]
max_temp <- max(med_temp_CO)
min_temp <- min(med_temp_CO)

print(paste0("The maximum median annual temperature of ", max_temp, " occurred in ", max_yr, " and the minimum median annual temperature of ", min_temp, " occurred in ", min_yr, " ."))
```
...
```{r}
sum_num_fun <- function(num){
  if(num < 10){
return(num)
  }else{
return(num %% 10 + sum_num_fun(num %/% 10))
  }
}

print(sum_num_fun(935068))
```
